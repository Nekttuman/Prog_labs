...
void log_not_found(const fig& figure, std::fstream& log) {
  log<<"points "<< figure.first.x << ":" << figure.first.y <<"  "<< figure.second.x << ":" << 
    figure.second.y << "  " << figure.third.x << ":" << figure.third.y << "  " <<
    figure.four.x << ":" << figure.four.y << "  " << figure.five.x << ":" << figure.five.y << " not a figure\n";
}
        ...
        for (int four = 0; four < n - 1; four++) {
          for (int five = four + 1; five < n; five++) {
            ...
            if (distance(cur_figure.first, cur_figure.third) != distance(points[four], points[five]) ||
              ...
              cur_figure.four = points[five];
              cur_figure.five = points[four];
              log_not_found(cur_figure, log);
              continue;
            }
